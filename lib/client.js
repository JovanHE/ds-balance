 ...
 * dynamic-plugin closure builtins), so — like the official plugins — this half
 * reaches the host through `ctx.remote.commands` and injects its stylesheet
 * directly via `document`.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const inject = ["remote", "remote.commands", "sessions", "timer"];

		/** Resolve the active session id from the `sessions` snapshot service. */
		function activeSessionId(sessions) {
			try {
				const snapshot = sessions?.list?.getSnapshot?.();
				if (snapshot?.current !== void 0) return snapshot.current;
				if (Array.isArray(snapshot?.ids) && snapshot.ids.length > 0) return snapshot.ids[0];
			} catch {
				// fall through
			}
			return void 0;
		...
		/** Run one host command, returning its text result (JSON here). */
		async function runCommand(ctx, line) {
			const sessionId = activeSessionId(ctx.sessions);
			if (sessionId === void 0) throw new Error("no active session — open a session first, then retry");
			// commands/execute expects (agentId, line, images, signal?): the
			// browser face always passes an empty attachment array here.
			const result = await ctx.remote.commands.execute(sessionId, line, []);
			if (!result.ok) throw new Error(`${result.error.code}: ${result.error.message}`);
			if (result.value === void 0) return `unknown command: ${line}`;
			return result.value.result?.text ?? "(no output)";
		}
						...
						runCommand(ctx, "/ds-balance")
							.then((text) => {
								...
								if (parsed && parsed.ok) {
									...
									setData(null);
									setError((parsed && parsed.error) || String(text));
									setPhase("error");
								}
							})
							.catch((err) => {
