...
const inject = ["shell", "credentials", "sandboxPolicy", "webServer"];
 ...
 *
 * Registers an HTTP route the bundle client fetches on its own origin. The
 * route answers with the normalized balance JSON. Routing through a plain
 * route — instead of a `/ds-balance` command — keeps the widget's 60s
 * auto-refresh out of the session log and conversation flow (a command
 * dispatch mints command/run + command/done records on every call).
 */
	...
	ctx.effect(() => webServer.register({
		...
		handler: async (req, res) => {
			try {
				...
				res.end(JSON.stringify({ ok: false, error: "INTERNAL", detail: String((err && err.message) || err) }));
			}
		}
	}));
}
