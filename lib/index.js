...
export function apply(ctx) {
	...
	commands.register({
		...
		handler: async () => {
			...
			// Commands must return a CommandResult ({ kind, text }), not a raw
			// string; the client parses `text` as JSON to drive the widget state.
			return { kind: data.ok ? "success" : "error", text: JSON.stringify(data) };
		}
	});
}
